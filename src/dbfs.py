...
def get_publish_logs(dbo):
    ...
    """
    return db.query(dbo, "SELECT ID, Name, Path FROM dbfs WHERE Path Like '/logs/publish%' ORDER BY Name DESC")
...
def get_document_repository(dbo):
    ...
    """
    return db.query(dbo, "SELECT ID, Name, Path FROM dbfs WHERE " \
        "Path Like '/document_repository%' ORDER BY Path, Name")

...
from sitedefs import SCALE_PDF, GHOSTSCRIPT_TEST_INSTALLED, GHOSTSCRIPT_SCALE_PDF
...
def get_media_by_seq(dbo, linktype, linkid, seq):
    ...
    rows = db.query(dbo, "SELECT * FROM media WHERE LinkTypeID = %d AND " \
        "LinkID = %d AND LOWER(MediaName) LIKE '%%.jpg' " \
...
def get_total_seq(dbo, linktype, linkid):
    return db.query_int(dbo, "SELECT COUNT(ID) FROM media WHERE LinkTypeID = %d AND " \
        "LinkID = %d AND LOWER(MediaName) LIKE '%%.jpg'" % (linktype, linkid))
...
def gs_installed():
    """
    Returns True if ghostscript is installed
    """
    return os.system(GHOSTSCRIPT_TEST_INSTALLED) == 0

def scale_pdf(filedata):
    """
    Scales the given PDF filedata down and returns the compressed PDF data.
    If the compressed PDF is larger than the original, uses the original instead.
    """
...
def scale_pdf_file(inputfile, outputfile):
    """
    Scale a PDF file using gs. Does it by rendering
    an image of the PDF then storing the image embedded in
    a (much smaller) PDF.
    """
    os.system(GHOSTSCRIPT_SCALE_PDF % { "output": outputfile, "input": inputfile})

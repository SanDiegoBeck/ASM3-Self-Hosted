...
def get_person(dbo, personid):
    ...
    """
    rows = db.query(dbo, get_person_query() + " WHERE o.ID = %d" % int(personid))
    if len(rows) == 0:
        return None
    else:
        return rows[0]
...
def get_person_similar(dbo, surname = "", forenames = "", address = ""):
    ...
    return db.query(dbo, get_person_query() + " WHERE OwnerSurname LIKE '%s'  AND OwnerAddress Like '%s%%'" %
        (surname.replace("'", "`"), address.replace("'", "`")))
...
def get_person_name_code(dbo, personid):
    ...
    """
    r = db.query(dbo, "SELECT OwnerName, OwnerCode FROM v_owner WHERE ID = %d" % int(personid))
    if len(r) == 0: return ""
...
def get_satellite_counts(dbo, personid):
    ...
    sql = "SELECT o.ID, " \
        ...
        "(SELECT COUNT(*) FROM animallost WHERE OwnerID = o.ID)) AS links " \
...
def get_reserves_without_homechecks(dbo):
    ...
    sql += " INNER JOIN adoption a ON a.OwnerID = o.ID " \
        "WHERE a.MovementType = 0 AND a.ReservationDate Is Not Null AND ReservationCancelledDate Is Null AND o.IDCheck = 0"
    return db.query(dbo, sql)
...
def get_links(dbo, pid):
    ...
    sql = "SELECT 'OO' AS TYPE, " \
        ...
        % ( _("Original Owner", l), linkdisplay, animalextra, int(pid), _("Brought In By", l), 
        linkdisplay, animalextra, int(pid),
        _("Owner Vet", l), linkdisplay, int(pid), _("Current Vet", l), linkdisplay, int(pid),
        _("Waiting List Contact", l), int(pid), _("Lost Animal Contact", l), int(pid),
...
def get_person_find_simple(dbo, query, classfilter="all", includeStaff = False, limit = 0, onlyindexed = False):
    ...
    if not includeStaff:
        sf = " AND IsStaff = 0"
    sql = unicode(get_person_query()) + " WHERE (" + u" OR ".join(ors) + ")" + cf + sf + " ORDER BY o.OwnerName"
...
def get_person_find_advanced(dbo, criteria, includeStaff = False, limit = 0):
    ...
    addwords("medianotes", "WebsiteMediaNotes")
    addcomp("filter", "aco", "IsACO = 1")
    addcomp("filter", "banned", "IsBanned = 1")
    addcomp("filter", "donor", "IsDonor = 1")
    addcomp("filter", "fosterer", "IsFosterer = 1")
    addcomp("filter", "homechecked", "IDCheck = 1")
    addcomp("filter", "homechecker", "IsHomeChecker = 1")
    addcomp("filter", "member", "IsMember = 1")
    addcomp("filter", "retailer", "IsRetailer = 1")
    addcomp("filter", "shelter", "IsShelter = 1")
    addcomp("filter", "staff", "IsStaff = 1")
    addcomp("filter", "giftaid", "IsGiftAid = 1")
    addcomp("filter", "vet", "IsVet = 1")
    addcomp("filter", "volunteer", "IsVolunteer = 1")
    if crit("filter") not in [ "all", "aco", "banned", "donor", \
        "fosterer", "homechecked", "homechecker", "member", "retailer", \
        "shelter", "staff", "giftaid", "vet", "volunteer" ]: 
        addstr("filter", "o.AdditionalFlags")
    if not includeStaff:
        c.append("IsStaff = 0")
    if len(c) == 0:
        sql = get_person_query() + " ORDER BY OwnerName"
    else:
        sql = get_person_query() + " WHERE " + " AND ".join(c) + " ORDER BY o.OwnerName"
    if limit > 0: sql += " LIMIT " + str(limit)
    return db.query(dbo, sql)

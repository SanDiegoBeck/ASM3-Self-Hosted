...
def get_reports(dbo):
    ...
    reps = [x for x in reps if len(x["HTMLBODY"]) > 3]
...
def get_criteria_params(dbo, customreportid, data):
    """
    Creates a list of criteria parameters to pass to a report. The data
    ...
    for name, rtype, question in crit:
        ...
        if rtype == "DATE":
            p.append( ( name , question, db.python2db(i18n.display2python(l, data[name])), data[name]) )  
        elif rtype == "STRING":
            p.append( ( name, question, utils.df_ks(data, name), utils.df_ks(data, name) ) )
        elif rtype == "NUMBER":
            p.append( ( name, question, data[name], data[name] ) )
        elif rtype == "ANIMAL" or rtype == "FSANIMAL" or rtype == "ALLANIMAL":
            p.append( ( name, i18n._("Animal", l), data[name], animal.get_animal_namecode(dbo, data[name])) )
        elif rtype == "PERSON":
            p.append( ( name, i18n._("Person", l), data[name], person.get_person_name(dbo, data[name])) )
        elif rtype == "LITTER":
            p.append( ( name, i18n._("Litter", l), data[name], data[name]) )
        elif rtype == "SPECIES":
            p.append( ( name, i18n._("Species", l), data[name], lookups.get_species_name(dbo, int(data[name]))) )
        elif rtype == "LOCATION":
            p.append( ( name, i18n._("Location", l), data[name], lookups.get_internallocation_name(dbo, int(data[name]))) )
        elif rtype == "TYPE":
            p.append( ( name, i18n._("Type", l), data[name], lookups.get_animaltype_name(dbo, int(data[name]))) )
    return p
...
def install_smcom_reports(dbo, user, ids):
    ...
    for r in reports:
        if r["ID"] in ids:
            ...
            insert_report_from_form(dbo, user, data)
...
def execute_title(dbo, title, username = "system", params = None):
    ...
    of parameters. username is the name of the user runnign the report.
...
class Report:
    ...
    isSubReport = False
    output = ""
    CURRENCY_FIELDS = "AMT AMOUNT DONATION DAILYBOARDINGCOST COSTAMOUNT COST"

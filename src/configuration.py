...
DEFAULTS = {
    ...
    "HelpingLostPetsURL": "www.helpinglostpets.com",
    "InactivityTimer": "No",
    ...
    "PetLinkBaseURL": "https://www.petlink.net/us/",
    "Pets911FTPURL": "members.pets911.com",
    "PicturesInBooks": "Yes",
    "PDFInline": "Yes",
    "PublisherUseCommentsForBlankNotes": "Yes",
    "PublisherIgnoreFTPOverride": "No",
    "QuicklinksID": "40,25,33,31,34,19,20",
...
def adoptapet_host(dbo):
    return cstring(dbo, "SaveAPetFTPURL")
...
def mapping_service_link_url(dbo):
    return cstring(dbo, "MappingServiceLinkURL", DEFAULTS["MappingServiceLinkURL"])

def mapping_service_embed_url(dbo):
    return cstring(dbo, "MappingServiceEmbeddedURL", DEFAULTS["MappingServiceEmbeddedURL"])
...
def meetapet_baseurl(dbo):
    return cstring(dbo, "MeetAPetBaseURL", DEFAULTS["MeetAPetBaseURL"])
...
def petfinder_host(dbo):
    return cstring(dbo, "PetFinderFTPURL")
...
def helpinglostpets_host(dbo):
    return cstring(dbo, "HelpingLostPetsFTPURL")
...
def petlink_baseurl(dbo):
    return cstring(dbo, "PetLinkBaseURL", DEFAULTS["PetLinkBaseURL"])

def pets911_host(dbo):
    return cstring(dbo, "Pets911FTPURL")
...
def publisher_use_comments_for_blank_notes(dbo):
    return cboolean(dbo, "PublisherUseCommentsForBlankNotes", DEFAULTS["PublisherUseCommentsForBlankNotes"] == "Yes")
...
def smarttag_host(dbo):
    return cstring(dbo, "SmartTagFTPURL")

def smarttag_user(dbo):
    return cstring(dbo, "SmartTagFTPUser")

def smarttag_password(dbo):
    return cstring(dbo, "SmartTagFTPPassword")
...
def rescuegroups_host(dbo):
    return cstring(dbo, "RescueGroupsFTPURL")

    ...
    var main = {
        ...
        tip: function() {
            var tips = [ 
                ...
                _("You can set a default amount for different donation types in the Settings-Lookup Data screen. Very handy when creating adoptions."),
        ...
        render_alerts: function() {
            ...
            var totalalerts = alerts.DUEVACC + alerts.DUETEST + alerts.DUEMED + alerts.URGENTWL +  
                alerts.LONGRSV + alerts.DUEDON + alerts.ENDTRIAL + alerts.NOTNEU + 
                alerts.NOTADOPT + alerts.LOOKFOR;
            ...
            if (totalalerts > 0) {
                ...
                if (alerts.DUEDON > 0 && common.has_permission("ovod")) {
                    s += '<a href="donation?offset=d0">' + html.icon("donation") + ' ' + 
                        common.ntranslate(alerts.DUEDON, [
                            _("{plural0} person has an overdue donation"),
                            _("{plural1} people have overdue donations"),
                            _("{plural2} people have overdue donations"),
                            _("{plural3} people have overdue donations")
                        ]) + '</a><br />';
                }
                ...
                if (alerts.NOTADOPT > 0 && common.has_permission("va")) {
        ...
        render_diary: function() {
            ...
            s += '<button id="button-adddiary">' + _("Add a diary note") + '</button></p>';
        ...
        render_stats: function() {
            ...
            if (stats.DONATIONS > 0 && common.has_permission("ovod")) {
                s += html.icon("donation") + ' ' + 
                    _("{0} received in donations").replace("{0}", format.currency(stats.DONATIONS))
                    + '<br />';
            }
        ...
        render: function() {
            ...
            _("ASM News"),
            '<span id="newsnav" class="ui-icon ui-icon-triangle-1-e" style="float: left; display: none"></span>',
            '</a>',
            '</p>',
            '<span id="newswrapper">',
            controller.news,
        ...
        bind: function () {
            ...
            // Hide the tips/quicklinks if options are set
            if (config.bool("DisableTips")) { $("#tips").hide(); }
            if (config.bool("DisableQuicklinks")) { $("#quicklinks").hide(); }
            if (config.bool("DisableTips") && config.bool("DisableQuicklinks")) { $("#linkstips").hide(); }

...
def reports(dbo):
    ...
    try:
        ...
        em = str(sys.exc_info()[0])
        al.error("FAIL: running batch tasks: %s" % em, "cron.reports", dbo, sys.exc_info())
...
def maint_db_diagnostic(dbo):
    try:
        ...
        em = str(sys.exc_info()[0])
        al.error("FAIL: uncaught error running maint_db_diagnostic: %s" % em, "cron.maint_animal_figures", dbo, sys.exc_info())

def maint_db_dump(dbo):
    try:
        print dbupdate.dump(dbo)
    except:
        em = str(sys.exc_info()[0])
